document.addEventListener('DOMContentLoaded', () => {
    ...
    const SPECIAL_CARDS = {
        PEEK: { id: 'PEEK_CARD', icon: '👁️', text: 'הצצה' },
        HINT: { id: 'HINT_CARD', icon: '💡', text: 'רמז נוסף' }
    };

    let topLevelCategories = [];
    const ACHIEVEMENTS = {
        FIRST_WIN: { id: 'FIRST_WIN', title: 'ניצחון ראשון', description: 'נצח את המשחק הראשון שלך.' },
        SPEEDSTER: { id: 'SPEEDSTER', title: 'מהיר בזק', description: 'סיים משחק בפחות מ-45 שניות.' },
        PERFECT_GAME: { id: 'PERFECT_GAME', title: 'משחק מושלם', description: 'קבל דירוג של 3 כוכבים בכל רמה.' },
        CATEGORY_EXPLORER: { id: 'CATEGORY_EXPLORER', title: 'חוקר קטגוריות', description: 'שחק ב-5 קטגוריות שונות.' },
        HINT_SAVER: { id: 'HINT_SAVER', title: 'חסכן רמזים', description: 'נצח משחק בינוני או קשה מבלי להשתמש ברמזים.' }
    };
    ...
    async function fetchCategories() {
        let allCategories = [];
        let page = 1;
        const baseUrl = 'https://dafbetzeva.co.il/wp-json/wp/v2/coloring-page-category?per_page=100&_fields=id,name,parent';
    ...
    async function fetchImages(categoryId, count = 8) {
        const url = `https://dafbetzeva.co.il/wp-json/wp/v2/coloring-pages?coloring-page-category=${categoryId}&_fields=yoast_head_json.og_image&per_page=${count}`;
        try {
    ...
    function addXP(amount) {
        ...
        while (playerXP >= xpToNextLevel) {
            ...
            playerLevel++;
            xpToNextLevel = calculateXPForLevel(playerLevel);
            showToast(`🎉 עלייה ברמה! אתה כעת רמה ${playerLevel}!`);
        }
        updatePlayerStatsUI();
        savePlayerStats();
    }
    ...
    function handleDailyChallengeClick() {
        ...
        if (lastCompletion === today) {
            showToast("כבר השלמת את אתגר היום! חזור מחר.");
            return;
        }
    ...
    function unlockAchievement(achievementId) {
        ...
        if (achievement) {
            showToast(`🏆 הישג נפתח: ${achievement.title}`);
        }
    }
    ...
    function showCategoryScreen() {
        ...
        gameHeader.querySelector('h1').textContent = 'זיכרון בכיף'; // Reset header
    ...
    async function initializeGame(categoryId, forcedDifficulty = null) {
        ...
        gameBoard.innerHTML = `
            <div class="loader-container" style="grid-column: 1 / -1;">
                ...
                <p class="loader-text">טוען תמונות...</p>
            </div>
        `;
        ...
        currentCategoryName = selectedCategory ? selectedCategory.name.replace(/דפי צביעה( של)?/gi, '').trim() : 'קטגוריה זו';
        let imageUrls = [];
        ...
        if (currentImageUrls.length < settings.pairs && currentImageUrls.length > 0) {
            ...
            gameBoard.innerHTML = `<p>לא נמצאו תמונות עבור קטגוריה זו או קטגוריות המשנה שלה. אנא בחר אחרת.</p>`;
            return;
        }
    ...
    function populateDropdown() {
        ...
        topLevelCategories.forEach(parentCategory => {
            ...
            parentOption.textContent = `${parentCategory.name.replace(/דפי צביעה( של)?/gi, '').trim()} ${parentStars}`;
            ...
            if (parentCategory.children.length > 0) {
                parentCategory.children.forEach(childCategory => {
                    ...
                    childOption.textContent = `  • ${childCategory.name.replace(/דפי צביעה( של)?/gi, '').trim()} ${childStars}`;
    ...
    async function fetchAndDisplayCategories() {
        ...
        if (flatCategories.length === 0) {
            ...
            categoryLoader.innerHTML = '<p class="loader-text">לא ניתן לטעון קטגוריות. אנא רענן.</p>';
    ...
    function updateHintDisplay() {
        ...
        if (hintSpan) {
            hintSpan.textContent = `רמז (${hintsRemaining})`;
        }
    }
    ...
    function showWinModal(elapsedTime) {
        ...
        const gameModeText = isDailyChallengeMode ? "האתגר היומי של היום" : `קטגוריה: ${currentCategoryName}`;
        const difficultyText = currentDifficulty === 'easy' ? 'קל' : currentDifficulty === 'medium' ? 'בינוני' : 'קשה';
        const formattedTime = formatTime(elapsedTime);
        shareableScoreText = `שיחקתי עכשיו בכיף הזיכרון! 🧠\n\n${gameModeText}\nרמה: ${difficultyText}\nניקוד: ${starsText}\nזמן: ${formattedTime}\nמהלכים: ${moves}\n\nהאם תוכלו לנצח את הניקוד שלי?`;
        ...
        winTimeElement.textContent = `הזמן שלך:  ${formattedTime}`;
        ...
        if (isDailyChallengeMode) {
            ...
            if (isNewRecord) {
                bestTimeElement.innerHTML = `✨ זמן שיא חדש! ✨`;
            } else if (bestTimeInSeconds) {
                const formattedBestTime = formatTime(bestTimeInSeconds * 1000);
                bestTimeElement.textContent = `הטוב ביותר בקטגוריה זו (${difficultyText}):  ${formattedBestTime}`;
            } else {
    ...
    function showHint() {
        ...
        if (pairToShow) {
            ...
            hintButton.disabled = true;
            hintSpan.textContent = 'אין רמזים!';
            setTimeout(() => {
    ...
    async function shareScore() {
        ...
        try {
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                ...
                showToast('הניקוד הועתק ללוח.');
            }
        } catch (error) {
            console.error('Error sharing score with image:', error);
            await navigator.clipboard.writeText(`${shareableScoreText}\n\nPlay here: ${linkToShare}`);
            showToast('שיתוף תמונות אינו נתמך. הניקוד הועתק ללוח.');
        }
    }
